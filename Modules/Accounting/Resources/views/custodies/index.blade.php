                            ...
                            <div class="form-group mr-2">
                                ...
                                <select name="account_id" id="account_id" class="form-control select2">
                                    ...
                                    @foreach ($accounts as $acc)
                                        <option value="{{ $acc->id }}" {{ ($account_id == $acc->id) ? 'selected' : '' }}>{{ $acc->display() }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="form-group mr-2">
                                ...
                                <tbody>
                                    @foreach ($custodies as $custody)
                                        <tr>
                                            <td>{{ $custody->id }}</td>
                                            <td>
                                                @if (auth()->user()->isAbleTo('accounts-read') && !is_null($custody->account))
                                                    <a href="{{ route('accounts.show', $custody->account) }}">{{ $custody->account->name }}</a>
                                                @else
                                                    {{ $custody->account->name }}
                                                @endif
                                            </td>
                                            <td>
                                                @if (auth()->user()->isAbleTo('accounts-read') && !is_null($custody->credit_account))
